...
@Component({
  selector: 'app-weeklydata',
  templateUrl: './weeklydata.component.html',
  styleUrls: ['./weeklydata.component.css']
})
export class WeeklydataComponent{
  Arraysymname:any = [];
  selected = "----";
  test="";
  ...
  async selectChange(filterVal: any) {
    if (filterVal != "-Select-")
    {
      ...
      let response = await this.ApiService.showTasksWeekly(temp[0]);
...
for (const key in Sres.final_data) {
  ...
  columnDefs = [
    ...
    { field: "5volume", sortable: true, filter: true, headerName: "Volume"}];
